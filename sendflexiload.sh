...
function copyright_msg() {
    ...
    echo
    echo "Copyright (c) Martin Wimpress, http://sendflexiload.com. MIT License"
    echo
...
function usage() {
    ...
    echo
    echo "    cd ~/"
    echo "    wget https://github.com/sendflexiload/raw/${VER}/`basename ${0}` -O `basename ${0}`"
...
if [ -e /etc/apt/sources.list.d/sendflexiload-java-${LSB_CODE}.list ]; then
    ncecho " [x] Removing ppa:sendflexiload/java "
    rm -v /etc/apt/sources.list.d/sendflexiload-java-${LSB_CODE}.list* >> "$log" 2>&1
    cecho success
fi
...
# Update apt cache
echo "deb file://${WORK_PATH}/deb / #Local Java - https://github.com/sendflexiload" > /etc/apt/sources.list.d/oab.list
apt_update
